...
export default class ProductService {
    ...
    private countByCategoryUri: string = import.meta.env.VITE_APP_API_PRODUCTS_COUNT_BY_CATEGORY
    ...
    async countByCategory(categoryName: string): Promise<number> {
        try {
            const response = await axios.get(`${this.countByCategoryUri}/${categoryName}`)
            const data: number = await response.data
            return data
        } catch (error) {
            throw new Error('Error with API calling: ' + error)
